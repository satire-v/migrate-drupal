...
class Directus {
  ...
  async createArticleValueSetQuery(article: DrupalArticle, categoryMap: Obj): Promise<string> {
    ...
    const b = this.drupal.createNewBar(title.slice(0, 20));
    ...
    const { imageID } = await this.drupal.drupalToDirectusImage(
      article.image_uri,
      article.relative_uri,
    );
    const body = mysql2.escape(await this.drupal.processHTMLInlineFileTags(article));
    const values = `('${pub}', 1, 1, '${created}', '${changed}', ${title}, ${body}, ${tags}, ${imageID}, ${caption}, ${teaser}, ${categoryID}, '${newSlug}', ${legacySlug})`;
    b && b.update(1);
    this.drupal.incrementMainBar();
    return values;
  }
}
