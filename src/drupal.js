...
class Drupal {
  ...
  async getAllArticles(): Promise<Array<DrupalArticle>> {
  ...
  async getDrupalCategoriesMap(): Promise<{ [string]: number }> {
  ...
  static downloadImage(fullUri: string): request {
    const options = {
      encoding: null,
      headers: {
        'user-agent': 'node.js',
    ...
    return request.get(fullUri, options).on('response', (data) => {
  ...
  async genManagedFileHTMLTag(fileObj: Obj): Promise<string> {
  ...
  static genReqFromSrc(src: string, relativeUri: string): { fileName: string, req: request } {
    let fileName = '';
    let req = null;
